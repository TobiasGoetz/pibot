...
class PiBot(discord.ext.commands.Bot):
    ...
    async def load_cogs(self) -> None:
        """ Load all cogs. """
        cogs = [p.stem for p in pathlib.Path("./cogs").glob("*.py")]
        for cog in cogs:
    ...
    async def on_message(self, message: discord.Message, /) -> None:
        ...
        for pref in prefixes:
            if message.content.lower().startswith(pref):
                default_command_channel = discord.utils.get(self.get_all_channels(), guild__name=message.guild.name,
                                                            name='botspam')
                command_channel = message.guild.get_channel(
                    await self.database.get_setting(message.guild, "command_channel")) or default_command_channel
                ...
                response = await message.channel.send(
                    embed=discord.Embed(
                        description=
                        f':no_entry_sign: **{message.author.name}** '
