...
| `DEEPL_API_KEY` | Optional | —             | —                                                             | Global DeepL API key used when a server has no `translations.deeplApiKey` override. Per-guild overrides via `/settings`.                      |
| `CLOUDFLARE_AI_URL` | Optional | —          | —                                                             | Global Cloudflare AI Gateway base URL (through `/compat`; the OpenAI client appends `/chat/completions`). Requires `CLOUDFLARE_AI_GATEWAY_TOKEN`. |
| `CLOUDFLARE_AI_GATEWAY_TOKEN` | Optional | — | —                                                             | Global Cloudflare AI Gateway token.                                                                                                         |
| `CLOUDFLARE_AI_MODEL` | Optional | `openai/gpt-4o-mini` | —                                    | Global Cloudflare model for `/summarize`.                                                                                                   |
