...
3. **Helm chart → GHCR (OCI)**
   ...
   - Install: `helm install <release-name> oci://ghcr.io/<owner-lowercase>/helm-charts/pibot --version <version>` (credentials via `secretRef.name`; non-secrets under `pibot:` in `charts/pibot/values.yaml`, rendered as `PIBOT_*` env vars). Optional in-cluster Valkey: `--set valkey.enabled=true` (upstream subchart; injects `PIBOT_VALKEY_URI`). Use `helm registry login ghcr.io` when the registry requires authentication.
